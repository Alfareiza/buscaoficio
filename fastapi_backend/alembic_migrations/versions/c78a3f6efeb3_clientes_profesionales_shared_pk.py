...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        'clientes_referido_por_id_fkey', 'clientes', 'clientes', ['referido_por_id'], ['usuario_id']
    )
...
def downgrade() -> None:
    ...
    op.add_column('profesionales', sa.Column('id', sa.UUID(), autoincrement=False, nullable=False))
    op.create_primary_key('profesionales_pkey', 'profesionales', ['id'])
    op.create_unique_constraint('profesionales_usuario_id_key', 'profesionales', ['usuario_id'])

    op.drop_constraint('clientes_referido_por_id_fkey', 'clientes', type_='foreignkey')
    op.drop_constraint('clientes_pkey', 'clientes', type_='primary')
    op.add_column('clientes', sa.Column('id', sa.UUID(), autoincrement=False, nullable=False))
    op.create_primary_key('clientes_pkey', 'clientes', ['id'])
    op.create_foreign_key(
        'clientes_referido_por_id_fkey', 'clientes', 'clientes', ['referido_por_id'], ['id']
    )
