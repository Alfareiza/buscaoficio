...
- **`OtpManager` (`app/otp_manager.py`)** — modeled directly on `RefreshTokenManager`'s shape. Codes are SHA-256 hashed, never stored raw, in the `email_otps` table (migration `a067ad066d81`). 10-minute expiry (`OTP_CODE_EXPIRE_SECONDS`), max 5 verify attempts, 60s resend cooldown. Like `refresh_tokens`, **rows are never purged** — same known cleanup gap, now duplicated across two tables.
...
- **Frontend:** `components/actions/otp-auth-action.ts` (Server Actions: `requestOtpAction`, `verifyOtpAction`, `registerClienteOtpAction`, `registerProfesionalOtpAction`) and `components/auth/AuthCard.tsx` (single client component driving a 4-step flow: `email → otp → onboarding-name → onboarding-role`, keyed by local `useState`, not a wizard library). `onboarding-role` is a radio list (filled inner disc when selected) plus pill **Crear cuenta** and **Volver** — **merged to `main` via PR #31** (issue #30). Profesional fields on that step: documento + searchable catalog multi-selects. `finish()` wraps `router.push("/dashboard")` in `useTransition` so button pending covers the App Router wait, not only the Server Action. `AuthCard` takes `mode: "page" | "modal"` (modal calls an `onSuccess` callback instead of `router.push`) and `intent?: "login" | "register"` (controls subtitle copy + the bottom toggle link only — the underlying flow is identical either way, since OTP doesn't know login vs. signup until verify).
...
- **`AuthBrandPanel` (`components/auth/AuthBrandPanel.tsx`)** is the decorative half of that layout. It stays a Server Component: gradient + card motion is CSS only. `SERVICIOS` is the catalog; optional `durationSeconds` overrides `DEFAULT_STEP_SECONDS` (9). Distinct holds get generated `@keyframes` in the component — do not hardcode 1/N percentages in `globals.css` again. Three slots walk the list in strides of 3; keep `SERVICIOS.length` coprime with 3 so every slot shows every card and no oficio is on screen twice. Icons live in `public/images/auth/icons/`.
...
| `app/global-error.tsx` | App Router | React render errors → Sentry. Stale Server Action Flight message → reload, no Sentry. |
...
- Frontend: Next.js `redirect()` throws internally in production, but a Jest mock of it does not — code depending on `redirect()` halting execution needs an explicit `return redirect(...)`, otherwise execution falls through under test (can end up calling a downstream function with an undefined token, e.g. `items-action.ts`).
...
### Registration
OTP `register_profesional_otp` requires `zona_ids` and `categoria_ids`
(existing catalog rows, min 1) and writes the N:M tables. Cliente OTP
